...
X_numpy = np.load('../data/underwater_hazy_512.npy')
Y_numpy = np.load('../data/underwater_clear_512.npy')
...
with tf.name_scope("generator_loss"):
    ...
    gen_loss = gen_loss_GAN * 10.0  +  gen_loss_L1 * 100.0 + gen_loss_gradient * 10.0 + gen_loss_trans * 50.0 + gen_loss_imq * 5.0
...
mode = "train"
if mode == 'train':
    saver.restore(sess, "./na_water_16_2/na_water_16_2.ckpt")
    batchsize = 4
    ...
    for step in range (10000000000):
        ...
        if step % 100 == 0:
            ...
            print (step)
            saver.save(sess, "./na_water_16_2/na_water_16_2.ckpt")
    ...
    for i in range (X_numpy_test.shape[0]):
        ...
        #im = batch_he_cpu(im)
        imjc,imjc2=sess.run([xcomr,xtr], feed_dict={X_raw:im, istrain:False})
        imjc = np.clip(imjc, 0, 255)
        imjc2 = np.clip(imjc2, 0, 255)
        target = Y_numpy_test[i].copy()
        out = np.hstack([im[0],imjc[0],target])
        #imsave("./output_all/"+str(i)+"result.png",out)
        imsave("./output_all/"+str(i)+"truth.png",target)
        imsave("./output_all/"+str(i)+"result.png",np.uint8(imjc[0]))
        imsave("./output_all/"+str(i)+"trans.png",imjc2[0,:,:,0])
        imsave("./output_all/"+str(i)+"input.png",np.uint8(im[0]))
        #com = im * (imjc2/255.0) + imjc * (1 - imjc2/255.0)
        #imsave("./output_all/"+str(i)+"com.png",np.uint8(com[0]))
        print (i)
elif mode == "one_pic":
    ...
    saver.restore(sess, "./na_water_16_2/na_water_16_2.ckpt")
    cap = cv2.VideoCapture('../data/crab.mp4')
    ...
    while 1:
        ...
        if ret == True:
            ...
            imjc,imt,aa = sess.run([xcomwr,xtr,xar], feed_dict={X_raw:judge,istrain:False})
            print (aa)
            imjc = np.clip(imjc, 0, 255)
            imjc = np.uint8(imjc)
